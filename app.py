...
# REMPLACER TOUT le bloc class Config par :
class Config:
    SECRET_KEY = os.getenv('SECRET_KEY', secrets.token_hex(32))
    is_production = bool(os.getenv('DATABASE_URL'))  # présent sur Render ET Railway
    ...
    if is_production:
        ...
        print(f"☁️  Production environment detected")
...
class Tunnel(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.public_url:
            base_domain = (os.getenv('RENDER_EXTERNAL_URL') or os.getenv('BASE_URL', '')).replace('https://', '').replace('http://', '')
            if base_domain:
...
<body>
    ...
    stats = {
        ...
        'base_url': (os.getenv('RENDER_EXTERNAL_URL') or os.getenv('BASE_URL') or 'http://localhost:8080')
    }
